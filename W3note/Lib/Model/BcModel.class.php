...
class BcModel extends RelationModel{
	  ...
	  public function getusername(){
		  if (isset ($_POST['username'])) {
			   if(in_array(trim($_POST['username']),array('网菠萝果','admin','管理员','博主'))){
		       return $data= '￣□￣';
			...
			if($_SESSION['loginUserName'])return $data='网菠萝果';
			}	
	    } 
	...
	public function content() {
		...
		if (isset ($_POST['content']) && !empty ($_POST['content'])) {
			 import("@.ORG.Input");
			 $data =Input::deleteHtmlTags($_POST['content']);
			 $data =Input::safeHtml($data);
			if (strlen($data) > 1000) {
	...
	public function geturl(){
		if (isset ($_POST['url'])) {
		import("@.ORG.Input");
		$data = Input::deleteHtmlTags($_POST['url']);
			 $data = Input::safeHtml($data);
			 //$data =delUrl($data);
	...
	*/
	 ...
	 function gclist($field,$where='',$pagesize=5) {
